  ...
  : describe("Test transactions from user on Vault on SHORT exposition", function () {
      ...
      describe("Full test on Long position", function () {
        before(async () => {
          ...
          await MyVault.setExposition(1); //1 for Long | 2 for short
        });
        describe("First deposit", function () {
          ...
          it("tests if user has spend correct amount of USDC", async function () {
            await expect(afterTx["user"][1]["usdc_balance"]).to.be.eq(beforeTx["user"][1]["usdc_balance"]-deposit1)
           });
           it("tests if user has received correct amount of PLP", async function () {
            await expect(afterTx["user"][1]["plp_balance"]).to.be.eq(parseInt(deposit1)*10**12)
            PLPbalanceT0 = (afterTx["user"][1]["plp_balance"]).toString()
           });
          it("tests on positions", async function () {
            await expect(afterTx["positions"]["short"]["size"]).to.be.eq(0)
            await expect(afterTx["positions"]["long"]["size"]/10**24).to.be.eq(parseInt(parseInt(deposit1*1.1).toFixed(0)))
            await expect(afterTx["positions"]["long"]["leverage"]).to.be.within(1.09, 1.11)
          });
        });
        describe("Withdraw total", function () {
          before(async () => {
            //user, vault contract, GMX_controller contract, USDC contract
            beforeTx = await getData(MyVault, GMX_controller, USDC, user)
            await withdrawStrat(user, PLPbalanceT0)
            afterTx = await getData(MyVault, GMX_controller, USDC, user)
          ...
          //  });
           it("tests if user has received correct amount of PLP", async function () {
            await expect(afterTx["user"][1]["plp_balance"]).to.be.eq(0)
           });
          it("tests on positions", async function () {
            await expect(afterTx["positions"]["short"]["size"]).to.be.eq(0)
