  ...
  : describe("Unit test for the LP Token", function () {
      ...
      describe("test actions on token accepted to deposit", function () {
        it("should not accept or remove a token if not the owner", async function () {
          await expect(
            MyVault.connect(accounts[1]).acceptToken(USDC.address)
          ).to.be.revertedWith("Ownable: caller is not the owner");
        });
        it("should not remove a token if not added", async function () {
          await expect(MyVault.removeToken(USDC.address)).to.be.revertedWith(
            "Token not yet added"
          );
        });
        it("should accept a new token if not already accepted", async function () {
          await expect(MyVault.acceptToken(USDC.address))
            .to.emit(MyVault, "NewTokenAdded")
            .withArgs(USDC.address);
          await expect(MyVault.acceptToken(USDC.address)).to.be.revertedWith(
            "Token already added"
          );
        });
        ...
        it("should not remove a token if not the owner", async function () {
          await expect(
            MyVault.connect(accounts[1]).removeToken(USDC.address)
          ).to.be.revertedWith("Ownable: caller is not the owner");
        });
        it("should remove a token", async function () {
          await expect(MyVault.removeToken(USDC.address))
            .to.emit(MyVault, "TokenRemoved")
            .withArgs(USDC.address);
        });
      });
      describe("test on deposit erc20", function () {
        ...
        it("should not deposit a token not accepted", async function () {
          await expect(
            MyVault.connect(accounts[1]).deposit(WETH.address, depositAmount)
          ).to.be.revertedWith("Token not yet supported");
        });
        it("should not deposit a token if amount <= 0", async function () {
          await expect(
            MyVault.connect(accounts[1]).deposit(USDC.address, 0)
          ).to.be.revertedWith("Amount to deposit is mandatory");
        });
        it("should deposit and receive LP Token", async function () {
          await USDC.connect(whale).approve(MyVault.address, depositAmount);
          expect(await USDC.allowance(whale.address, MyVault.address)).to.be.eq(
            depositAmount
          );
